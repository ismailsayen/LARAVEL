<x-master location='Profile'>
    <div class="flex gap-4 rounded-xl ">
        ...
        <div
            class="max-w-sm h-[700px] overflow-auto p-6 bg-gray-500 border border-gray-700 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <h1>Profile:</h1>
            <hr>
            <h5 class="mb-2 text-2xl font-semibold tracking-tight text-white">{{ $profile->name }}</h5>
            <img class="w-full h-1/2 rounded-lg " src="{{ asset('storage/' . $profile->image) }}"
                alt="{{ $profile->name }}">
        ...
        <div class=" w-full h-[700px]  rounded-lg bg-gray-500 border border-gray-700 p-6    overflow-auto">
            <h1>Publications:</h1>
            <hr>
